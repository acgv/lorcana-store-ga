  ...
  const fetchAllCardPrices = async (comparisons: ComparisonResult[]) => {
    if (!comparisons || comparisons.length === 0) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "No hay cartas para buscar precios",
      })
      return
    }

    setFetchingAllPrices(true)
    setFetchProgress({ current: 0, total: comparisons.length })
    ...
    // Procesar cartas una por una con delay para evitar rate limiting
    for (let i = 0; i < comparisons.length; i++) {
      const comp = comparisons[i]
      
      // Saltar si ya tiene precio
      if (comp.marketPriceUSD) {
        setFetchProgress({ current: i + 1, total: comparisons.length })
        continue
      }
      ...
      try {
        ...
        if (result.success && result.data?.marketPriceUSD) {
          successCount++
        ...
        console.error(`Error buscando precio para ${comp.cardName}:`, error)
        failCount++
      }

      // Delay entre solicitudes para evitar rate limiting (3 segundos)
    ...
    toast({
      title: "✅ Búsqueda masiva completada",
      description: `${successCount} precios encontrados, ${failCount} no encontrados de ${comparisons.length} cartas`,
    })
  }
